...
export function TermsOfUseSection({ standalone = false }: Props) {
  const { locale, t } = useI18n();
  const [html, setHtml] = useState<string | null>(null);
  const [sections, setSections] = useState<LegalSection[]>([]);
  const [loading, setLoading] = useState(true);
  const title = t("footer.terms");
    ...
    >
      ...
      >
        {loading ? (
          ...
          <LegalDocumentBody sections={sections} listStyle="numbered" />
        )}
      </div>
      <p className="mt-3 text-center text-xs text-gray-500">{t("legal.scrollTerms")}</p>
    </section>
  );
}
