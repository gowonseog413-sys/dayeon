...
const labelClass = "mb-1 block text-xs font-semibold text-[var(--pink-deep)] sm:text-sm";
const inputClass =
  "mt-0 w-full rounded-xl border-2 border-[var(--pink-border)] bg-[var(--pink-bg-soft)] px-3 py-2.5 text-sm outline-none focus:border-[var(--pink-accent)] focus:bg-white";
      ...
      >
        ...
        <div
          className={`grid grid-cols-1 gap-4 ${
            form.channelType === "credit_card" ? "sm:grid-cols-3" : "sm:grid-cols-2"
          }`}
        >
          <label className="block min-w-0 text-sm">
            <span className={labelClass}>결제 유형</span>
            <select
          ...
          {form.channelType === "credit_card" && (
            <label className="block min-w-0 text-sm">
              <span className={labelClass}>카드 브랜드</span>
              <select
        ...
        {form.channelType === "credit_card" && (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <label className="block min-w-0 text-sm">
              <span className={labelClass}>카드 번호 (끝 4자리)</span>
              <input
                required
                maxLength={4}
            ...
            <label className="block min-w-0 text-sm sm:col-span-1">
              <span className={labelClass}>카드 소유자명</span>
              <input
                required
            ...
            <label className="block min-w-0 text-sm">
              <span className={labelClass}>만료 월 (MM)</span>
              <input
                required
                maxLength={2}
            ...
            <label className="block min-w-0 text-sm">
              <span className={labelClass}>만료 연도 (YY)</span>
              <input
                required
                maxLength={2}
        ...
        {form.channelType === "gopay" && (
          <label className="block text-sm">
            <span className={labelClass}>GoPay 연동 휴대폰</span>
            <input
              required
        ...
        {form.channelType === "virtual_account" && (
          <label className="block text-sm">
            <span className={labelClass}>가상계좌 은행</span>
            <select
              value={form.vaBank}
