...
export type PopupTimezone = (typeof POPUP_TIMEZONES)[keyof typeof POPUP_TIMEZONES];

/** ERP 관리 화면 기본 시간대 */
export const EVENT_POPUP_ADMIN_TZ: PopupTimezone = POPUP_TIMEZONES.KR;

const TZ_OFFSET: Record<PopupTimezone, string> = {
  [POPUP_TIMEZONES.KR]: "+09:00",
  [POPUP_TIMEZONES.ID]: "+07:00",
};

export function normalizeEventTime(raw: string | undefined, fallback = "00:00") {
  const v = (raw || "").trim();
...
export function combineEventDateTime(
  ...
  fallbackTime: string,
  timeZone: PopupTimezone = EVENT_POPUP_ADMIN_TZ,
) {
...
export function eventPopupTodayKey(
  now = new Date(),
  timeZone: PopupTimezone = EVENT_POPUP_ADMIN_TZ,
) {
...
export function isEventPopupActive(
  ...
  const start = combineEventDateTime(popup.startDate, popup.startTime, "00:00", timeZone);
...
export type EventPopupStatus = "active" | "scheduled" | "expired" | "disabled" | "invalid";
